...
class FeditNativeApp:
    ...
    def _get_clips_in_rect(self, rect_min, rect_max):
        ...
        for idx, track in enumerate(self.sequencer.tracks):
            clip_top = self.wheel_graph_height + idx * track_height + 20
            clip_bottom = self.wheel_graph_height + (idx + 1) * track_height - 5
    ...
    def update_loop(self):
        ...
        if mouse_left_down:
            if mouse_left_just_pressed and ctrl_down and self._is_mouse_in_timeline_viewport(mpos):
    ...
    def render_timeline(self):
        ...
        for i, track in enumerate(self.sequencer.tracks):
            ...
            for clip in track.clips:
                ...
                fill_col = (base_col[0], base_col[1], base_col[2], 150)
                if clip == self.sequencer.selected_clip:
                    border_col = (255, 255, 255)
                elif clip in self.multi_selected_clips:
                    border_col = (255, 215, 0)
                    fill_col = (255, 215, 0, 180)
                else:
                    border_col = base_col

                dpg.draw_rectangle([x_start, y_offset + 20], [x_start + width, y_offset + track_height - 5], color=border_col, thickness=2, fill=fill_col, parent="timeline_canvas")
