...
1) Stop any running instance to avoid file locks:
   `Get-Process Fedit2 -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue`
...
4) Launch for testing: `Start-Process -FilePath dist/Fedit2.exe`
5) For source-run during development: `./.venv/Scripts/python.exe native_app.py`

Artifacts: the built binary is at `dist/Fedit2.exe`.
 */